...
from Backent.models import admindb, categorydb, productdb,contactdb
...
def productsave(req):
    if req.method == "POST":
        ...
        si = req.POST.get('size')
        co = req.POST.get('color')
        im= req.FILES['image']
        obj = productdb(Name=na,Category_id=ca,Description=de,Quantity=qu,Price=pr,Size=si,Color=co,Image=im)
        obj.save()
